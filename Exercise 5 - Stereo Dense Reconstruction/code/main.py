...
left_img = cv2.resize(cv2.imread('../data/left/000000.png'),
                      (-1, -1), fx=0.5, fy=0.5)
left_img = cv2.cvtColor(left_img, cv2.COLOR_BGR2GRAY)
right_img = cv2.resize(cv2.imread('../data/right/000000.png'),
                       (-1, -1), fx=0.5, fy=0.5)
...
# Parts 1, 2, 4: Disparity on one image pair
...
R = np.asarray([[0, -1,  0],
                [0,  0, -1],
                [1,  0,  0]])
p_F_points = np.transpose(R.T @ p_C_points.T)

plt.figure()
ax = plt.axes(projection='3d')
ax.scatter3D(p_F_points[:, 0], p_F_points[:, 1], p_F_points[:, 2], intensities)
...
# repmat(single(intensities(1: 10:end))'/255, [1 3]), '
# filled
# ');
# axis
# equal;
...
# T_W_F = T_W_C * [R_C_frame zeros(3, 1);
# zeros(1, 3)
# 1];
...
# dlmwrite('points.ply', ...
# [all_points
# ' repmat(single(all_intensities)', [1 3])], ...
# '-append', 'delimiter', ' ');